...
// Game data (includes image paths and detail properties)
const gameData = {
  disturbed: {
    title: "Disturbed",
    description:
...
function renderGameGrid() {
  ...
  Object.entries(gameData).forEach(([id, game]) => {
    const card = document.createElement("div");
    card.className = "game-card";
    card.onclick = () => showGameDetail(id);

    const thumbnailDiv = document.createElement("div");
    thumbnailDiv.className = "game-thumbnail";
    const img = document.createElement("img");
    img.src = game.thumbPath || "placeholder.jpg";
...
function renderDownloadButtons(game) {
  ...
  const availablePlatforms = Object.entries(game.downloads || {}).filter(
    ([, link]) => link
  );

  if (availablePlatforms.length === 0) {
    // Display a message if no downloads are available
    const message = document.createElement("p");
    message.textContent = "Download link will be available soon!";
  ...
  availablePlatforms.forEach(([platform, link]) => {
    ...
    // Get icon SVG, default to an empty string if not found
    const icon = downloadIcons[platform] || "";

    // Capitalize the platform name for the button text
    const platformName = platform.charAt(0).toUpperCase() + platform.slice(1);

    button.innerHTML = `${icon} Download for ${platformName}`;
    container.appendChild(button);
  });
}

// Show game detail (MODIFIED to include dynamic sidebar and download buttons)
function showGameDetail(gameId) {
  ...
  // Update game detail sidebar (Details)
